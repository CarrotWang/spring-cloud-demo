...
### Sleuth
Sleuth是“分布式服务追踪系统”，它跟踪一个用户请求的过程，通过它我们可以看到请求背后的调用链视图，它是“调试”和“监控”微服务的关键工具。

“分布式服务追踪系统”涉及的主要内容有“数据采集”，“数据传输”，“数据存储”，“数据分析”，“数据可视化”。
...
“分布式服务追踪系统”的始祖是Google的Dapper，并且谷歌根据它发表了论文。

下面了解下Dapper的名词：

（1）Span

最基本的工作单元。例如：发送一个RPC就是一个新的span，同样一次RPC的应答也是。Span通过一个唯一的，长度64位的ID来作为标识，另一个64位ID用于跟踪。Span也可以带有其他数据，例如：描述，时间戳，键值对标签，起始Span的ID，以及处理ID（通常使用IP地址）等等。 Span有起始和结束，他们跟踪着时间信息。span应该都是成对出现的，有失必有终，所以一旦创建了一个span，那就必须在未来某个时间点结束它。 提示： 起始的span通常被称为：root span。它的id通常也被作为一个跟踪记录（Trace）的id。

（2）Trace

一个树结构的Span集合。例如：在分布式大数据存储中，可能每一次请求都是一次跟踪记录。

（3）Annotation

用于记录一个事件时间信息。（ https://my.oschina.net/roccn/blog/834278 ）

![Feign](https://github.com/CarrotWang/spring-cloud-demo/blob/master/imgs/Sleuth.png?raw=true "Feign")

上图中，每一种颜色，代表一个span。，整体是一个trace。

###### 抽样采集

理论上来说，收集的跟踪信息越多就可以越好反映出系统的实际运行情况，并给出更精确的预警和分析。但在高并发的分布式系统运行时，大量的请求调用会产生海量的跟踪日志信息，如果收集过多的跟踪信息将会对整个分布式系统的性能造成一定的影响，同时保存大量的日志信息也需要不少的存储开销。所以，在Sleuth中采用了“抽象收集”的方式来跟踪信息打上标记，也就是我们前面第四个布尔值，它代表了该信息是否要被后续的跟踪信息收集器获取和存储。
